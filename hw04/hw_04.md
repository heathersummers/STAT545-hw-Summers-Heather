...
[General data reshaping and relationship to aggregation](#general-data-reshaping-and-relationship-to-aggregation) - [Make your own tidyr cheatsheet](#make-your-own-tidyr-cheetsheet) - [Make a tibble with one row per year and columns for life expectancy for two or more countries](#make-a-tibble-with-one-row-per-year-and-columns-for-life-expectancy-for-two-or-more-countries)

[Join, merge, look up](#join-merge-look-up) - [Create a second data frame, complementary to Gapminder and join this with Gapminder](#create-a-second-data-frame-complementary-to-Gapminder-and-join-this-with-Gapminder)
...
We get this warning message because the `country` columns in both data sets are not just characters but are factors. To get rid of the warning message and convert the columns `country` to characters used the function `as.character()`.
