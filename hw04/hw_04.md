...
<a href="#top">Back to top</a> For this activity I will compare the life expectancy of Canada, New Zealand, France, Belgium, and Denmark. First, select data for only those countries using the function `filter()` and then use the function `select()` to retain only the columns for country, year, and life expectancy.
...
<a href="#top">Back to top</a> Below is a simple cheatsheet I created of the different `join` functions. Dataset 'a':
