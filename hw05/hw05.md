...
![](hw05_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-15-1.png) I used the color scales in the package `viridis` to colour-code the countries since the colours are easier to read by those with colourblindness. I also used the function `fct_reorder2()` so that for my factor colour the legend appears in the same order as the data.
