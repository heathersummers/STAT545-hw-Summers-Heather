...
``` r
gapminder %>%
    filter(country %in% c("Canada", "China","France", "Belgium", "Denmark")) %>% 
...
``` r
fiveCountries <- c("Canada", "China","France", "Belgium", "Denmark")
...
![](hw02_files/figure-markdown_github-ascii_identifiers/unnamed-chunk-22-2.png)
